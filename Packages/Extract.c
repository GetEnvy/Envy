...
void GetInstallDirectory()
{
	...
	// Get the Envy install directory from the registry
	// Check for "Path hack" first @ http://shareazawiki.anenga.com/tiki-index.php?page=FAQ%3AMiscellaneous
	//lRet = RegOpenKeyEx( HKEY_CURRENT_USER, L"Software\\Envy\\Envy", 0, KEY_QUERY_VALUE, &hKey );

	if ( skinType == typeData )
	{
		lRet = SHGetValue( HKEY_CURRENT_USER, L"Software\\Envy\\Envy", L"DataPath",
	...
	lRet = SHGetValue( HKEY_CURRENT_USER, L"Software\\Envy\\Envy", L"Path",
		&dwType, (LPBYTE)skins_dir, &dwBufLen );
	if ( lRet != ERROR_SUCCESS )
	{
