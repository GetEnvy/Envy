...
CLibraryFile* CLibraryMaps::LookupFile(DWORD nIndex, BOOL bSharedOnly /*0*/, BOOL bAvailableOnly /*0*/) const
{
...
CLibraryFile* CLibraryMaps::LookupFileByName(LPCTSTR pszName, QWORD nSize, BOOL bSharedOnly, BOOL bAvailableOnly) const
{
	...
	return ( m_pNameMap.Lookup( PathFindFileName( pszName ), pFile ) &&
...
CLibraryFile* CLibraryMaps::LookupFileByPath(LPCTSTR pszPath, BOOL bSharedOnly, BOOL bAvailableOnly) const
{
	...
	return ( m_pPathMap.Lookup( pszPath, pFile ) &&
		pFile->CheckFileAttributes( SIZE_UNKNOWN, bSharedOnly, bAvailableOnly ) ) ?
		pFile : NULL;
}
...
CLibraryFile* CLibraryMaps::LookupFileByHash(const CEnvyFile* pFilter, BOOL bSharedOnly, BOOL bAvailableOnly) const
{
	CQuickLock oLock( Library.m_pSection );
...
CFileList* CLibraryMaps::LookupFilesByHash(const CEnvyFile* pFilter, BOOL bSharedOnly, BOOL bAvailableOnly, int nMaximum) const
{
	CQuickLock oLock( Library.m_pSection );
