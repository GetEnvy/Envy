...
// http://getenvy.com/archives/envywiki/Developers.Code.CG1Neighbour.html
	...
	: CNeighbour( PROTOCOL_G1, pBase )	// First, call the CNeighbour constructor
{
	...
	if ( Settings.Gnutella1.VendorMsg && m_bVendorMsg )
	{
		...
		pVendor->WriteShortLE( 0x0003 );
		pVendor->WriteShortLE( 1 );
		pVendor->WriteLongLE( 'YVNE' );	// "ENVY" for Envy
		pVendor->WriteShortLE( 0x0001 );
		pVendor->WriteShortLE( 1 );
		pVendor->WriteLongLE( 'YVNE' );
		pVendor->WriteShortLE( 0x0002 );
		pVendor->WriteShortLE( 1 );
		pVendor->WriteLongLE( 'YVNE' );
		pVendor->WriteShortLE( 0x0003 );
...
BOOL CG1Neighbour::OnVendor(CG1Packet* pPacket)
{
	...
	else if ( nFunction == 0xFFFF )			// The packet has vendor or function numbers, and the 2 bytes of function are all 1s
	{
		...
		// Vendor is the ASCII text "ENVY" for Envy
		else if ( nVendor == 'ENVY' || nVendor == 'YVNE' )	// Backwards for network byte order (Confirm?)
		{
			// Function code query for "ENVY" (do)
			CG1Packet* pReply = CG1Packet::New( pPacket->m_nType, 1, pPacket->m_oGUID );	// Create a reply packet
			pReply->WriteLongBE( 'ENVY' );
			pReply->WriteShortLE( 0xFFFE );
			pReply->WriteShortLE( 1 );
...
{
	// Only do this if the remote computer is running Envy and the settings here allow custom vendor message packets
...

	{
		...
		for ( CHostCacheIterator i = HostCache.Gnutella1.Begin(); i != HostCache.Gnutella1.End() && nCount < 20; ++i )
		{
			...
			// If host is running Envy compatible, was added recently, and we can connect to it (do)
			if ( pHost->m_pVendor && pHost->m_pVendor->m_bExtended &&	// If this host is running Envy/PeerProject/Shareaza, and
				...
				pHost->CanConnect( tNow ) )								// We can connect to it now (do)
			{
				...
				if ( ! pPacket )
				{
					...
					pPacket->WriteLongBE( 'ENVY' ); 	// Back-compatability, vendor code should be "ENVY" for Envy ("RAZA" for Shareaza?)
	...
	if ( pPacket && nCount )
	{
		...
		//pPacket->EnvySign();							// Obsolete placeholder does nothing (do)
...
{
	// EnvyVerify placeholder does nothing, and always returns false (do)
	//if ( ! pPacket->EnvyVerify() )
	//	return FALSE;
...
BOOL CG1Neighbour::OnPush(CG1Packet* pPacket)
{
	...
	if ( pOrigin && pPacket->Hop() )	// Calling Hop moves 1 from TTL to hops
	{
		...
		// Otherwise it's running Gnutella2 software like Envy
