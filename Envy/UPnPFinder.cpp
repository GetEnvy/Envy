...
// Walks through all port mappings and searches for "Envy" string.
...
void CUPnPFinder::DeleteExistingPortMappings(ServicePointer pService)
{
	...
	do
	{
		...
		hr = InvokeAction( pService, L"GetGenericPortMappingEntry", strInArgs, strActionResult );
		...
		if ( SUCCEEDED( hr ) && ! strActionResult.IsEmpty() )
		{
			...
			// VT_BSTR	PortMappingDescription = "Envy TCP"
			...
			if ( _tcsistr( strActionResult, L"|VT_BSTR=Envy TCP|" ) != NULL ||
				 _tcsistr( strActionResult, L"|VT_BSTR=Envy UDP|" ) != NULL )
			{
