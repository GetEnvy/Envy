...
void CEDPacket::WriteFile(const CEnvyFile* pEnvyFile, QWORD nSize,
	const CEDClient* pClient, const CEDNeighbour* pServer, bool bPartial)
{
	...
	// Filename
	CEDTag( ED2K_FT_FILENAME, pEnvyFile->m_sName ).Write( this, bUnicode, bSmlTags );
		...
		if ( m_bUDP )
		{
						...
						if ( m_nLength >= 16 )
						{
							...
							if ( m_nLength >= 24 )
							{
								...
								if ( m_nLength >= 28 )
								{
									...
									strOutput.AppendFormat( L", udp flags: %s", (LPCTSTR)GetED2KServerUDPFlags( nUDPFlags ) );
									if ( m_nLength >= 32 )
									{
			...
			case ED2K_S2CG_SEARCHRESULT:
				if ( m_nLength >= Hashes::Ed2kHash::byteCount + 4 + 2 + 4 )
				{
					...
					strOutput.AppendFormat( L"hash: %s, id: %u, port: %u, tags: %s", (LPCTSTR)oMD4.toString(), nID.s_addr, nPort,
			...
			case ED2K_S2C_IDCHANGE:
				if ( m_nLength >= 4 )
				{
					...
					if ( m_nLength >= 8 )
					{
						...
						strOutput.AppendFormat( L", tcp flags: %s", (LPCTSTR)GetED2KServerTCPFlags( nFlags ) );
						if ( m_nLength >= 12 )
						{
			...
			case ED2K_C2S_OFFERFILES:
				if ( m_nLength >= 4 )
				{
					...
					if ( nCount && m_nLength >= 4 + Hashes::Ed2kHash::byteCount + 4 + 2 + 4 )
					{
						...
						strOutput.AppendFormat( L" -> hash: %s, id: %u, port: %u, tags: %s%s", (LPCTSTR)oMD4.toString(), nID.s_addr, nPort,
		...
		else if ( m_nEdProtocol == ED2K_PROTOCOL_EDONKEY )
		{
			...
			case ED2K_C2C_HELLOANSWER:
				if ( m_nLength >= Hashes::Guid::byteCount + 4 + 2 + 4 + 2 + 4 )
				{
					...
					strOutput.Format( L"guid: %s, id: %u, port: %u, tags: %s, server: %s:%u", (LPCTSTR)oGUID.toString(), nClientID, nPort,
						(LPCTSTR)CEDTag::ToString( m_pBuffer + Hashes::Guid::byteCount + 4 + 2, m_nLength - ( Hashes::Guid::byteCount + 4 + 2 ) ),
		...
		else if ( m_nEdProtocol == ED2K_PROTOCOL_EMULE )
		{
			...
			case ED2K_C2C_REQUESTPARTS_I64:
				if ( m_nLength >= Hashes::Ed2kHash::byteCount + 8 * 6 )
				{
					...
					strOutput.Format( L"hash: %s, offsets: %I64u-%I64u, %I64u-%I64u, %I64u-%I64u", (LPCTSTR)oMD4.toString(), offset[0], offset[3], offset[1], offset[4], offset[2], offset[5] );
				}
				break;
