	...
	CNeighbour* pNeighbour)
{
	...
	if ( pNeighbour->m_nProtocol == PROTOCOL_G2 ||
		 ...
		 pNeighbour->m_sUserAgent.Find( L"Envy" ) == 0 )	// ToDo: Is this still correct ?
		nBits = 1;
...
void CQueryHashTable::MakeKeywords(const CString& strPhrase, CStringList& oKeywords)
{
	...
	for ( ; nPos < strPhrase.GetLength() ; ++nPos )
	{
			...
			|| ( nKanaType[ 0 ] && boundary[ 0 ] != boundary[ 1 ] ) )
		{
			...
			// but Envy (Shareaza 2.2+) is not really following GDF about
...
void CQueryHashTable::AddFile(const CEnvyFile& oFile)
{
	if ( ! m_pHash )
		return;
...
void CQueryHashTable::AddHashes(const CEnvyFile& oFile)
{
	if ( ! m_pHash )
		return;
