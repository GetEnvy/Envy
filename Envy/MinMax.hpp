...
const bool ENVY_RESTRICT_WP64 = true;
const bool ENVY_ADVANCED_MIN_TEMPLATE = true;
...
namespace min_max_detail
{
	...
	//		( ! ENVY_RESTRICT_WP64 && sizeof( primary ) < 8 ),
...
typename min_max_detail
	::MinMaxResultRef< T, U, ENVY_ADVANCED_MIN_TEMPLATE >::type
inline min(T& lhs, U& rhs)
{
	typedef min_max_detail
		::MinMaxResultRef< T, U, ENVY_ADVANCED_MIN_TEMPLATE >::type Result;
	if ( rhs < lhs )
		return Result( rhs );
	else
		return Result( lhs );
...
template<typename T, typename U>
typename min_max_detail
	::MinMaxResult< T, U, ENVY_ADVANCED_MIN_TEMPLATE >::type
inline min(const T& lhs, U& rhs)
{
	typedef min_max_detail
		::MinMaxResult< T, U, ENVY_ADVANCED_MIN_TEMPLATE >::type Result;
	if ( rhs < lhs )
		return Result( rhs );
	else
		return Result( lhs );
